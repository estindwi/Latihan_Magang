...
<div class="bg-white p-6 rounded shadow max-w-xl">
  ...
  <form action="{{ route('rekam-medis.update', $rekamMedis->id) }}" method="POST" class="space-y-4">
    @csrf
    @method('PUT')

    @php
      $oldPasien = old('pasien_id', $rekamMedis->pasien_id);
      $oldDokter = old('dokter_id', $rekamMedis->dokter_id);
    @endphp
    ...
    <div>
      ...
      <select id="pasien_id" name="pasien_id" class="border p-2 w-full rounded">
        ...
        @foreach($pasiens as $p)
          <option value="{{ $p->id }}" {{ (string)$oldPasien === (string)$p->id ? 'selected' : '' }}>
            {{ $p->nama }} ({{ $p->nik }})
          </option>
        @endforeach
      </select>
    ...
    <div>
      ...
      <select id="dokter_id" name="dokter_id" class="border p-2 w-full rounded">
        ...
        @foreach($dokters as $d)
          <option value="{{ $d->id }}" {{ (string)$oldDokter === (string)$d->id ? 'selected' : '' }}>
            {{ $d->nama }} - {{ $d->spesialis }}
          </option>
        @endforeach
      </select>
    ...
    <div>
      ...
      <textarea name="keluhan" class="border p-2 w-full rounded">{{ old('keluhan', $rekamMedis->keluhan) }}</textarea>
    </div>
    ...
    <div class="flex gap-2">
      ...
      <a href="{{ route('rekam-medis.index') }}" class="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">
        Batal
      </a>
    </div>
  </form>
